<template>
  ...
  const changePassword = () => {
    ...
    try {
    // 从服务器获取当前密码
    const response = await axios.get<PasswordResponse>(
      'http://47.104.150.153:7000/user/login/password'
    );
    const currentPassword = response.data.password;

    // 检查新密码是否与当前密码不同
    if (newEntry.value === currentPassword) {
      alert('新密码不能与当前密码相同');
      return;
    }

    // 发送新密码到服务器进行更新
    await axios.post('http://47.104.150.153:7000/user/login/password', {
      newPassword: newEntry.value,
    });
